{
  "_args": [
    [
      {
        "raw": "rev-path@^1.0.0",
        "scope": null,
        "escapedName": "rev-path",
        "name": "rev-path",
        "rawSpec": "^1.0.0",
        "spec": ">=1.0.0 <2.0.0",
        "type": "range"
      },
      "/Applications/MAMP/htdocs/jump/wp-content/themes/dayone/node_modules/gulp-rev"
    ]
  ],
  "_from": "rev-path@>=1.0.0 <2.0.0",
  "_id": "rev-path@1.0.0",
  "_inCache": true,
  "_location": "/gulp-rev/rev-path",
  "_nodeVersion": "0.12.3",
  "_npmUser": {
    "name": "sindresorhus",
    "email": "sindresorhus@gmail.com"
  },
  "_npmVersion": "2.9.1",
  "_phantomChildren": {},
  "_requested": {
    "raw": "rev-path@^1.0.0",
    "scope": null,
    "escapedName": "rev-path",
    "name": "rev-path",
    "rawSpec": "^1.0.0",
    "spec": ">=1.0.0 <2.0.0",
    "type": "range"
  },
  "_requiredBy": [
    "/gulp-rev"
  ],
  "_resolved": "https://registry.npmjs.org/rev-path/-/rev-path-1.0.0.tgz",
  "_shasum": "d4ccb436ac3370c4607175ce88eafc5c65c5d653",
  "_shrinkwrap": null,
  "_spec": "rev-path@^1.0.0",
  "_where": "/Applications/MAMP/htdocs/jump/wp-content/themes/dayone/node_modules/gulp-rev",
  "author": {
    "name": "Sindre Sorhus",
    "email": "sindresorhus@gmail.com",
    "url": "sindresorhus.com"
  },
  "bugs": {
    "url": "https://github.com/sindresorhus/rev-path/issues"
  },
  "dependencies": {
    "modify-filename": "^1.0.0"
  },
  "description": "Create a revved file path",
  "devDependencies": {
    "ava": "0.0.4"
  },
  "directories": {},
  "dist": {
    "shasum": "d4ccb436ac3370c4607175ce88eafc5c65c5d653",
    "tarball": "https://registry.npmjs.org/rev-path/-/rev-path-1.0.0.tgz"
  },
  "engines": {
    "node": ">=0.10.0"
  },
  "files": [
    "index.js"
  ],
  "gitHead": "fc5df89bace86b3c667503ccbcd840867ed5c67b",
  "homepage": "https://github.com/sindresorhus/rev-path",
  "keywords": [
    "rev",
    "revving",
    "web",
    "file",
    "name",
    "filename",
    "path",
    "cache",
    "caching"
  ],
  "license": "MIT",
  "maintainers": [
    {
      "name": "sindresorhus",
      "email": "sindresorhus@gmail.com"
    }
  ],
  "name": "rev-path",
  "optionalDependencies": {},
  "readme": "ERROR: No README data found!",
  "repository": {
    "type": "git",
    "url": "git+https://github.com/sindresorhus/rev-path.git"
  },
  "scripts": {
    "test": "node test.js"
  },
  "version": "1.0.0"
}
